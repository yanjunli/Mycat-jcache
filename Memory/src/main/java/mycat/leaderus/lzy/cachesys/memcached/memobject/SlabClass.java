...
class SlabClass {
    ...
    private static long getSlabClass(SlabClass tmpSlabClass, int flags, int bytesSizes, long timeout, byte[] data) {
        ...
        if (size <= MemConfig.CHUNK_SIZES) {
            ...
            if (size < (index = (int) Math.floor(Math.log(MemConfig.SLAB_SIZE / MemConfig.CHUNK_SIZES) / Math.log(MemConfig.FACTOR))) * MemConfig.CHUNK_SIZES) {
                index = (int) Math.floor(Math.log(size / MemConfig.CHUNK_SIZES) / Math.log(MemConfig.FACTOR));
            } else {
        ...
        if (tmpSlabClass.slabclass[index].size() == 0) {
            ...
            if (slab != -1) {
                tmpSlabClass.slabclass[index].addLast(slab);
                return (long) tmpSlabClass.slabClassInx << 48 | (long) tmpSlabClass.slabs[slab].slabInx << 32 | tmpSlabClass.slabs[slab].init(index).setChunk(flags, bytesSizes, timeout, data);
            }
            ...
            if ((j = tmpSlabClass.getSlab()) != -1) {
                ...
                return (long) tmpSlabClass.slabClassInx << 48 | (long) tmpSlabClass.slabs[j].slabInx << 32 | tmp;
            } else {
                return getSlabClass(new SlabClass((short) slabClassTotal.getAndIncrement()), flags, bytesSizes, timeout, data);
            }
        }
    ...
    public static long setBuffer(int flags, int bytesSizes, long timeout, byte[] data) {
        ...
        if (all.size() == 0) {
            ...
            long tmp2;
            for (int i = 0; i < tmp; i++) {
                if ((tmp2 = getSlabClass(all.get(i), flags, bytesSizes, timeout, data)) != -1)
                    return tmp2;
            }
            return getSlabClass(new SlabClass((short) slabClassTotal.getAndIncrement()), flags, bytesSizes, timeout, data);
        }
    ...
    public static boolean setBuffer(int flags, int bytesSizes, long timeout, byte[] data, long index) {
        ...
        return all.get(i).slabs[j].setChunk(flags, bytesSizes, timeout, data, k);
    }
    ...
    public static boolean setBufferWithCAS(int flags, int bytesSizes, long timeout, byte[] data, long index, long CAS) {
        ...
        short j = (short) ((index & 0x7fff00000000L) >> 32);
        int k = (int) (index & 0x7fffffff);
        return all.get(i).slabs[j].setChunkWithCAS(flags, bytesSizes, timeout, data, k, CAS);
    }
    ...
    public static int getBufferLength(long index) {
        ...
        short j = (short) ((index & 0x7fff00000000L) >> 32);
        int k = (int) (index & 0x7fffffff);
        return all.get(i).slabs[j].getLengthInt(k);
    }

    public static void clear() {
        for (int i = 0; i < all.size(); i++) {
            SlabClass sc = all.get(i);
