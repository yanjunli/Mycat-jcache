...
public class BinaryGetQCommand implements Command{
	...
	public void execute(Connection conn) throws IOException {
		...
		if (extlen == 0 && bodylen == keylen && keylen > 0) {
			...
			System.out.println("执行getq 命令   key: "+keystr);
			...
			BinaryResponseHeader header = buildHeader(conn.getBinaryRequestHeader(),BinaryProtocol.OPCODE_GETQ,keystr.getBytes(),value,extras,1l);
